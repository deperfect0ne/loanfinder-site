...
export async function generateMetadata({
  ...
  return {
    title: dict.creditors.ferratum.title,
    description: dict.creditors.ferratum.description,
    openGraph: {
      title: dict.creditors.ferratum.title,
      description: dict.creditors.ferratum.description,
      url: "/ferratum",
...
export default async function FerratumPage({
  params,
}: {
  params: Promise<{ lang: string }>
}) {
  const { lang } = await params
  const dict = await getDictionary(lang)
  const creditor = dict.creditors.ferratum
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <section aria-labelledby="creditor-info" className="my-12">
        <article>
          <Card className="overflow-hidden">
            <CardHeader className="bg-primary text-white">
              <CardTitle id="creditor-info" className="text-2xl">{creditor.name}</CardTitle>
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <section aria-labelledby="loan-details-title">
                  <h2 id="loan-details-title" className="font-semibold text-lg mb-4">{dict.common.loanDetails}</h2>
                  <ul className="space-y-2">
                    <li className="flex items-start">
                      <Check className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                      <span>{dict.common.amount}: {creditor.amount}</span>
                    </li>
                    <li className="flex items-start">
                      <Check className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                      <span>{dict.common.term}: {creditor.term}</span>
                    </li>
                    <li className="flex items-start">
                      <Check className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                      <span>{dict.common.interest}: {creditor.interest}</span>
                    </li>
                  </ul>
                </section>

                <section aria-labelledby="benefits-title">
                  <h2 id="benefits-title" className="font-semibold text-lg mb-4">{dict.common.benefits}</h2>
                  <ul className="space-y-2">
                    {creditor.benefits.map((benefit, i) => (
      ...
      <section className="my-16" aria-labelledby="calculator-title">
        <h2 id="calculator-title" className="text-3xl font-bold mb-8">{dict.common.calculatorTitle}</h2>
        <LoanCalculator lang={lang} creditor="ferratum" />
      </section>
      ...
      <section className="my-16" aria-labelledby="faq-title">
        ...
        <FaqSection lang={lang} category="ferratum" />
      </section>
