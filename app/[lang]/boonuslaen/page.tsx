...
export async function generateMetadata({
  ...
  return {
    title: dict.creditors.boonuslaen.title,
    description: dict.creditors.boonuslaen.description,
    openGraph: {
      title: dict.creditors.boonuslaen.title,
      description: dict.creditors.boonuslaen.description,
      url: "/boonuslaen",
...
export default async function BoonuslaenPage({
  params,
}: {
  params: Promise<{ lang: string }>
}) {
  const { lang } = await params
  const dict = await getDictionary(lang)
  const creditor = dict.creditors.boonuslaen
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <section aria-labelledby="boonuslaen-info" className="my-12">
        <article>
          <Card className="overflow-hidden">
            <CardHeader className="bg-primary text-white">
              <CardTitle id="boonuslaen-info" className="text-2xl">
                {creditor.name}
              </CardTitle>
      ...
      <section className="my-16" aria-labelledby="calculator-title">
        ...
        <LoanCalculator lang={lang} creditor="boonuslaen" />
      </section>
      ...
      <section className="my-16" aria-labelledby="faq-title">
        ...
        <FaqSection lang={lang} category="boonuslaen" />
      </section>
