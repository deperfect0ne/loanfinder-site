...
export default async function ConsumerLoanPage({
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <article>
        <section className="my-12" aria-label="Описание потребительского кредита">
          <SeoTextBlocks lang={lang} content={pageData.sisu} />
        </section>

        <section className="my-16" aria-label="Калькулятор потребительского кредита">
          <h2 className="text-3xl font-bold mb-8">{dict.common.calculatorTitle}</h2>
          <LoanCalculator lang={lang} loanType="consumer" />
        </section>

        <section className="my-16" aria-label="Кредиторы">
          <h2 className="text-3xl font-bold mb-8">{dict.common.creditorsTitle}</h2>
          <CreditorsList lang={lang} loanType="consumer" />
        </section>
